...
const styles = {
  socket: (on: boolean) => css({
    ...
    alignItems: 'center',
    boxShadow: `inset 0 0 7px rgba(0, 0, 0, 0.2), ${ on ? '0 0 1px #25292f' : '0 0 6px 2px #545454' }`,
    borderRadius: 1,
    cursor: 'pointer',
  }),
  switch: (on: boolean) => css({
    width: '35%',
    height: '50%',
    background: '#444',
    position: 'relative',
...
export const Switch = (p: {
  ...
  <div id='switch-plate' {...styles.socket(p.on)} onClick={p.onClick}>
    <div id='switch' {...styles.switch(p.on)} />
  </div>
);
